                    ...
                    <tbody style="font-size: 13px;">
                        ...
                        <?php foreach ($quote_items as $quote_item): ?>
							...
							<tr>
								...
								<td style="text-align:center; width:15%;vertical-align:middle;"><?= $quote_item->subtotal!=0?$this->erp->formatMoney($quote_item->subtotal):$free; ?></td>
								<?php
								if ($Settings->product_discount) {
                                    $percentage = '%';
                                    $discount = $quote_item->discount;
                                    $dpos = strpos($discount, $percentage);
                                    echo '<td style="width: 100px; text-align:right; vertical-align:middle;">' .($dpos == true ? '<small>('.$discount.')</small>' : '').' '. $this->erp->formatMoney($quote_item->discount) . '</td>';
                                }
                                if ($Settings->tax1) {
