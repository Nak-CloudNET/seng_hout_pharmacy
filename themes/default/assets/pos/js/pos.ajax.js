...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var pos_head2 = '</h4><p>Room | Table: <span style="font-size:18px;">'+ $("#suspend_name").val() +'</span><br/>Waiting No: <span style="font-size:18px;">'+ (parseFloat(queue ? queue : 0) + 1) +'</span><br/>Username: '+u_username+'<br/>Customer: '+$('#select2-chosen-1').text()+'<br>In: '+formattedDate+'<br/></p>';
		...
		$.each(positems, function (i, e) {
			...
			if(java_applet == 1) {
				...
				if(item_type_by!="")
				{
					...
					{
						var oprTr = '<tr style="font-weight:bold" class="row_' + item_id + ' order_print_food " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td>#'+(an-1)+'   ' + item_name + (item_note? '['+item_note+']':"") +'</td><td>' + formatDecimal(item_qty) +'</td></tr>';
					    $("#order-table-food").append(oprTr);
					   
					}else if(item_type_by=='drink' && item_print==0){
						var oprTr = '<tr style="font-weight:bold" class="row_' + item_id + ' order_print_drink " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td>#'+(an-1)+'   ' + item_name + (item_note? '['+item_note+']':"") +'</td><td>' + formatDecimal(item_qty) +'</td></tr>';
