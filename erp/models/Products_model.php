...
class Products_model extends CI_Model
{
	...
	public function getUsingStockProducts($term, $warehouse_id, $plan = NULL, $address = NULL, $limit = 100)
    {
        ...
        $this->db->limit($limit);
		if ($plan) {
			$this->db->select('products.*, warehouses_products.quantity as qoh, units.name as unit_name, (COALESCE(project_plan.project_qty,0) - COALESCE(using_stock.using_qty, 0)) as project_qty, project_plan.product_id as in_plan');
		} else {
	...
	public function getPlanUsing($plan_id, $product_code, $address)
	{
		...
		$this->db->select('project_plan_items.*, using_stock.using_qty')
	...
	{
        $this->db
            ->select('adjustment_items.*, purchase_items.expiry as adj_expiry, products.code,products.name,product_variants.name as variants')
			->join('products', 'products.id = adjustment_items.product_id','left')
			->join('purchase_items', 'purchase_items.transaction_id = adjustment_items.id','left')
			->join('product_variants', 'product_variants.product_id = products.id 
				AND product_variants.id = adjustment_items.option_id','left' )
			->where('purchase_items.transaction_type', 'ADJUSTMENT')
